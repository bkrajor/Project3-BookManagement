...
const createReview = async function (req, res) {
    try {
        ...
        await bookModel.findOneAndUpdate({ _id: paramBookId }, { $inc: { reviews: 1 } })
...
const updateReview = async function (req, res) {
    try {
        ...
        const updateReview = await reviewModel.findByIdAndUpdate({ _id: reviewId }, obj , { new: true })
...
const deleteReview = async function (req, res) {
    try {
        ...
        if (!validObjectId(bookId)) return res.status(400).send({ status: false, message: "Invalid bookId" })
        const isBookPresent = await bookModel.findOne({ _id: bookId, isDeleted: false })
        if (!isBookPresent) return res.status(400).send({ status: false, message: "No book present or the book is already deleted " })

        // ------------Validating reviewId-------------
        if (!validObjectId(reviewId)) return res.status(400).send({ status: false, message: "Invalid reviewId" })
        const isReviewPresent = await reviewModel.findOne({ _id: reviewId, isDeleted: false })
        if (!isReviewPresent) return res.status(400).send({ status: false, message: "No review present or the review is already deleted" })

        // ------------Matching review's bookId with bookId------------ 
        if (isReviewPresent.bookId != bookId) return res.status(400).send({ status: false, message: "BookId in review doesn't matched with given book" })
        
        //-------------Adding isDeleted and deletedAt keys in reviewData-------------- 
        isReviewPresent.isDeleted = true
        isReviewPresent.deletedAt = new Date()
